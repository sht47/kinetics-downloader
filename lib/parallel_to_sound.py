...
class Pool:
  ...
  """
    def __init__(self, classes, source_directory, target_directory,
                 num_workers, failed_save_file, no_sound_save_file):
        ...
        if self.classes is None:
            ...
            for filename in videos:
                ...
                self.videos_queue.put(
                    (video_id, video_path, self.target_directory, target_path))
        else:
            for class_name in self.classes:
                source_class_dir = os.path.join(self.source_directory,
                                                class_name.replace(" ", "_"))
                target_class_dir = os.path.join(self.target_directory,
                                                class_name.replace(" ", "_"))
                ...
                if os.path.isdir(source_class_dir):
                    ...
                    for filename in videos:
                        ...
                        target_path = os.path.join(target_class_dir,
                                                   "{}.mp3".format(video_id))
                        self.videos_queue.put((video_id, video_path,
                                               target_class_dir, target_path))
    ...
    """
        ...
        if self.no_sound_save_file is not None:
            self.no_sound_worker = Process(target=write_failed_worker,
                                           args=(self.no_sound_queue,
                                                 self.no_sound_save_file))
            self.no_sound_worker.start()

        # start download workers
        for _ in range(self.num_workers):
            worker = Process(target=sound_worker,
