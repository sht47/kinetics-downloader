...
def download_classes(classes, num_workers, failed_save_file, compress, verbose,
  ...
  """
    ...
    for list_path, save_root in zip(
        ...
        pool = parallel.Pool(classes=classes,
                             videos_dict=data,
                             directory=save_root,
    ...
    if args.all:
        ...
        for category in categories:
            download_category(category, args.num_workers, args.failed_log,
                              args.compress, args.verbose, args.skip,
                              args.log_file)
        ...
        if args.test:
            # download the test set
            download_test_set(args.num_workers, args.failed_log, args.compress,
                              args.verbose, args.skip, args.failed_log)


if __name__ == "__main__":

    parser = argparse.ArgumentParser(
        "Download Kinetics videos in the mp4 format.")
    ...
    parser.add_argument("--all",
                        action="store_true",
                        help="download the whole dataset")
    parser.add_argument("--test",
                        action="store_true",
                        help="download the test set")

    parser.add_argument("--num-workers",
                        type=int,
                        default=1,
                        help="number of downloader processes")
    parser.add_argument("--failed-log",
                        default="dataset/failed.txt",
                        help="where to save list of failed videos")
    ...
    parser.add_argument(
        ...
        "--log-file",
        help=
        "log file for youtube-dl (the library used to download YouTube videos)"
    )
