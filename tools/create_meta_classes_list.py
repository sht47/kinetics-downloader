...
def main(args):
    ...
    if args.format == constants.FORMAT_VIDEOS:
        ...
        train_videos = metadata.get_valid_frames(videos,
                                                 config.TRAIN_FRAMES_ROOT)
    elif args.format == constants.FORMAT_SOUND:
        train_videos = metadata.get_valid_sound(videos,
                                                config.TRAIN_SOUND_ROOT)
    else:
    ...
    if args.format == constants.FORMAT_VIDEOS:
        validation_videos = metadata.get_valid_videos(videos,
                                                      config.VALID_ROOT)
    elif args.format == constants.FORMAT_FRAMES:
        validation_videos = metadata.get_valid_frames(videos,
                                                      config.VALID_FRAMES_ROOT)
    elif args.format == constants.FORMAT_SOUND:
        validation_videos = metadata.get_valid_sound(videos,
                                                     config.VALID_SOUND_ROOT)
    else:
        raise ValueError("Invalid format type.")
    ...
    if not args.validation_from_training:
        ...
        if args.format == constants.FORMAT_VIDEOS:
            test_videos = metadata.get_valid_videos(videos,
                                                    config.TEST_ROOT,
                                                    class_dirs=False)
        elif args.format == constants.FORMAT_FRAMES:
            test_videos = metadata.get_valid_frames(videos,
                                                    config.TEST_FRAMES_ROOT,
                                                    class_dirs=False)
        elif args.format == constants.FORMAT_SOUND:
            test_videos = metadata.get_valid_sound(videos,
    ...
    if not args.force:
        if sorted(train_videos.keys()) != sorted(validation_videos.keys()):
            raise ValueError(
                "Training and validation videos do not contain the same classes."
            )
    ...
    set_train = {
        cls: videos
        for cls, videos in train_videos.items() if cls in classes
    }
    ...
    if args.validation_from_training:
        ...
        set_test = {
            cls: videos
            for cls, videos in validation_videos.items() if cls in classes
        }
        ...
        for cls in classes:
            ...
            num_validation_videos = math.ceil(
                num_videos * args.validation_from_training_fraction)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Create metadata for all downloaded videos and specify which classes to include."
    )

    parser.add_argument("format",
                        help="{}, {} or {}".format(constants.FORMAT_VIDEOS,
                                                   constants.FORMAT_FRAMES,
                                                   constants.FORMAT_SOUND))
    parser.add_argument("classes",
                        help="path to a JSON list of classes to include")
    parser.add_argument("save_path", help="where to save the metadata")

    parser.add_argument("--max-training-videos",
                        default=None,
                        type=int,
                        help="maximum number of training videos per class")
    parser.add_argument("--max-testing-videos",
                        default=None,
                        type=int,
                        help="maximum number of testing videos per class")
    parser.add_argument("--validation-from-training",
                        default=False,
                        action="store_true",
                        help="create the validation set from the training set")
    parser.add_argument(
        "--validation-from-training-fraction",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "create metadata even if training and validation sets do not contain the same classes"
    )
